...
try {
  ...
  const artifactIdentity=await fileIdentity(artifact);
  const result={schemaVersion:2,profile:profileId,artifact,artifactIdentity,targetUrl,mode,completed:true,success:true,titleScreen:title,runtime,consoleMessages,exceptions,failedResources,siblingFileRequests,blobUrls,chromeOutput:chromeOutput.join("").slice(-10000),capturedAt:new Date().toISOString()};
